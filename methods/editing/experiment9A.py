# Experiment 9A: Tracking hallucinations one by one (4.1.1)
...
def run_main(model_type, remove_hallucinations = True, remove_gt = False, weight_factor = 1, sample_size = 500, output_file="experiment9A_results.pt"):
  ...
  if os.path.exists(output_file_path):
    ...
    print("No file detected. Create new results")
    results = dict()
  ...
  img_count = 0
  cache = dict()
  for coco_img in tqdm(sampled_coco_img_ids, desc=f"Experiment 9A: one-by-one editing"):
    img_count += 1
    if img_count % 100 == 0:
    ...
    if remove_hallucinations:
      for caption_word, coco_class in set(data[coco_img]["chair_evals"]["mscoco_hallucinated_words"]):
        text_embeddings.append((caption_word, get_phrase_embedding(caption_word, vocab_embeddings, tokenizer)))
    ...
    for (caption_word, text_embedding) in text_embeddings:
      text_embeddings = text_embedding.unsqueeze(0)
...
def parse_args():
  parser = argparse.ArgumentParser(description="Experiment 9A")
...
if __name__ == "__main__":
  ...
  output_file = f"experiment9A_{model_type}_w{weight}_ss{sample_size}_h{hallucinations}_gt{gt}_{experiment_name}.pt"
