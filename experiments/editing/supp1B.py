# Supplemental method 1B: for each image, compute a set of embeddings based on cosine similarity such that when you subtract out the vocab token from the embedding, the caption loses the word
...
from src.caption.instruct_blip_engine import InstructBLIPVicuna13B
from experiments.blip_utils import seq_to_tokens, get_phrase_embedding
import torch
import pickle
...
def check_if_word_exists(remove_indices, image_embeddings, target_word, img_id):
    ...
    for i in remove_indices:
        inputs_embeds[:, i] -= (torch.norm(inputs_embeds[:, i]) / 2) * get_phrase_embedding(target_word, vocab_embeddings)
...
progress_bar = tqdm(total=len(rand_img_ids), desc="Supp1B Processing")
for img_id in rand_img_ids:
    ...
    if img_count % 10 == 0:
        torch.save(img_to_embed_sets, "supp1B_results.pt")
